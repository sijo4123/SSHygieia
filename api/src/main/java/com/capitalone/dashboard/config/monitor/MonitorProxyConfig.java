	...
	public PasswordAuthentication proxyPasswordAuthentication() {
		if(StringUtils.isNotBlank(monitorProxySettings.getHost())) {
			...
			if(StringUtils.isBlank(username) || StringUtils.isBlank(password)) {
				throw new IllegalArgumentException("When enabling proxy access for the Monitor widget, username and password is required. "
						+ " Please ensure monitor.proxy.username and monitor.proxy.password are supplied.");
			}
